    ...
    <>
      <Router>
        ...
        <Route
          ...
          render={({ match }) => (
            <Catalog itemFiltered={data.filter((v) => String(v.typeCar) === String(match.params.typeCar))} />
          )} />
        <Route exact path="/catalog/:typeCar" render={({ match }) => (<Catalog data={data} filteredData={data.filter((item) => (item.typeCar) === (match.params.typeCar))} />)} />
        <Footer />
