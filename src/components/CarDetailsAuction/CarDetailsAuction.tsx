...
const CarDetailsAuction = ({ dataAuction = [] }: Props) => {
  ...
  const [item, setItem] = useState<AuctionResponse>()
    ...
    <>
      <Container fluid className="pb-5">
        <div className="p-5">
          ...
          <h6 className="mt-3">{item?.car.Title}</h6>
        </div>
        <Row>
          <Col>
            <Carousel>
              {item.car.images &&
                item.car.images.map((i) => {
                  return (
                    <div>
                      <img src={i.url} alt="car" className="car-details-img" />
                    </div>
                  )
                })}
            </Carousel>
          ...
          <Col>
            ...
            <Row className="mt-4 row-modal">
              <Col lg={6} md={12} sm={12} xs={12} className="font-weight-bold">
                {' '}
                {t('year')}:
              </Col>
